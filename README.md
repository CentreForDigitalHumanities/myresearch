...
3. Build the Docker images:
   ...
   ```
   docker compose run --rm django-dev python manage.py graphql_schema
   ```
...
4. The database will automatically be initialized, but you may want to fill it with
   ...
   ```bash
   docker compose run --rm django-{dev|prod} python manage.py create_dev_data  # add `--force` for prod
   ```
