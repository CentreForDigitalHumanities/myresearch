...
const emits = defineEmits<{
  (e: "update:variables", value: UnwrapNestedRefs<GraphQLListVariables>): void;
...
function updateFilterValues(newFilterValues: UUListTypes.FilterValues) {
  ...
  for (const [key, value] of Object.entries(newFilterValues)) {
    if (variables.value[key] !== value) {
      (variables.value as Record<string, UUListTypes.FilterValue>)[key] = value;
    }
  }
}
</script>
...
<template>
  ...
  >
    <template #data="{ data, isLoading }">
      <slot name="data" :data="data as ItemType[]" :is-loading="isLoading" />
    </template>
