# Generated by Django 5.0.14 on 2025-08-29 08:32

import django.contrib.postgres.fields
import django.db.models.deletion
from django.conf import settings
from django.db import migrations, models


class Migration(migrations.Migration):

    initial = True

    dependencies = [
        migrations.swappable_dependency(settings.AUTH_USER_MODEL),
    ]

    operations = [
        migrations.CreateModel(
            name="DateQuestion",
            fields=[
                (
                    "id",
                    models.BigAutoField(
                        auto_created=True,
                        primary_key=True,
                        serialize=False,
                        verbose_name="ID",
                    ),
                ),
                ("text", models.CharField(max_length=200)),
                ("text_en", models.CharField(max_length=200, null=True)),
                ("text_nl", models.CharField(max_length=200, null=True)),
                (
                    "step_order",
                    models.PositiveIntegerField(
                        blank=True,
                        help_text="The order of this question within the step.",
                        null=True,
                    ),
                ),
                (
                    "description",
                    models.TextField(
                        blank=True,
                        help_text="Context about the question and its purpose, which will be shown to the user.",
                    ),
                ),
                (
                    "description_en",
                    models.TextField(
                        blank=True,
                        help_text="Context about the question and its purpose, which will be shown to the user.",
                        null=True,
                    ),
                ),
                (
                    "description_nl",
                    models.TextField(
                        blank=True,
                        help_text="Context about the question and its purpose, which will be shown to the user.",
                        null=True,
                    ),
                ),
                ("required", models.BooleanField(default=False)),
                ("future_only", models.BooleanField(default=False)),
            ],
            options={
                "abstract": False,
            },
        ),
        migrations.CreateModel(
            name="FileUploadQuestion",
            fields=[
                (
                    "id",
                    models.BigAutoField(
                        auto_created=True,
                        primary_key=True,
                        serialize=False,
                        verbose_name="ID",
                    ),
                ),
                ("text", models.CharField(max_length=200)),
                ("text_en", models.CharField(max_length=200, null=True)),
                ("text_nl", models.CharField(max_length=200, null=True)),
                (
                    "step_order",
                    models.PositiveIntegerField(
                        blank=True,
                        help_text="The order of this question within the step.",
                        null=True,
                    ),
                ),
                (
                    "description",
                    models.TextField(
                        blank=True,
                        help_text="Context about the question and its purpose, which will be shown to the user.",
                    ),
                ),
                (
                    "description_en",
                    models.TextField(
                        blank=True,
                        help_text="Context about the question and its purpose, which will be shown to the user.",
                        null=True,
                    ),
                ),
                (
                    "description_nl",
                    models.TextField(
                        blank=True,
                        help_text="Context about the question and its purpose, which will be shown to the user.",
                        null=True,
                    ),
                ),
                ("required", models.BooleanField(default=False)),
                ("size_limit", models.PositiveIntegerField()),
            ],
            options={
                "abstract": False,
            },
        ),
        migrations.CreateModel(
            name="MRFormConfig",
            fields=[
                (
                    "id",
                    models.BigAutoField(
                        auto_created=True,
                        primary_key=True,
                        serialize=False,
                        verbose_name="ID",
                    ),
                ),
                ("name", models.CharField(max_length=200)),
                ("version", models.CharField(max_length=100)),
                ("created_at", models.DateTimeField(auto_now_add=True)),
                ("updated_at", models.DateTimeField(auto_now=True)),
            ],
        ),
        migrations.CreateModel(
            name="NumberQuestion",
            fields=[
                (
                    "id",
                    models.BigAutoField(
                        auto_created=True,
                        primary_key=True,
                        serialize=False,
                        verbose_name="ID",
                    ),
                ),
                ("text", models.CharField(max_length=200)),
                ("text_en", models.CharField(max_length=200, null=True)),
                ("text_nl", models.CharField(max_length=200, null=True)),
                (
                    "step_order",
                    models.PositiveIntegerField(
                        blank=True,
                        help_text="The order of this question within the step.",
                        null=True,
                    ),
                ),
                (
                    "description",
                    models.TextField(
                        blank=True,
                        help_text="Context about the question and its purpose, which will be shown to the user.",
                    ),
                ),
                (
                    "description_en",
                    models.TextField(
                        blank=True,
                        help_text="Context about the question and its purpose, which will be shown to the user.",
                        null=True,
                    ),
                ),
                (
                    "description_nl",
                    models.TextField(
                        blank=True,
                        help_text="Context about the question and its purpose, which will be shown to the user.",
                        null=True,
                    ),
                ),
                ("required", models.BooleanField(default=False)),
                ("positive_only", models.BooleanField(default=False)),
            ],
            options={
                "abstract": False,
            },
        ),
        migrations.CreateModel(
            name="SelectQuestion",
            fields=[
                (
                    "id",
                    models.BigAutoField(
                        auto_created=True,
                        primary_key=True,
                        serialize=False,
                        verbose_name="ID",
                    ),
                ),
                ("text", models.CharField(max_length=200)),
                ("text_en", models.CharField(max_length=200, null=True)),
                ("text_nl", models.CharField(max_length=200, null=True)),
                (
                    "step_order",
                    models.PositiveIntegerField(
                        blank=True,
                        help_text="The order of this question within the step.",
                        null=True,
                    ),
                ),
                (
                    "description",
                    models.TextField(
                        blank=True,
                        help_text="Context about the question and its purpose, which will be shown to the user.",
                    ),
                ),
                (
                    "description_en",
                    models.TextField(
                        blank=True,
                        help_text="Context about the question and its purpose, which will be shown to the user.",
                        null=True,
                    ),
                ),
                (
                    "description_nl",
                    models.TextField(
                        blank=True,
                        help_text="Context about the question and its purpose, which will be shown to the user.",
                        null=True,
                    ),
                ),
                ("required", models.BooleanField(default=False)),
                (
                    "multiple",
                    models.BooleanField(
                        default=False,
                        help_text="Allows multiple options to be selected if true.",
                    ),
                ),
            ],
            options={
                "abstract": False,
            },
        ),
        migrations.CreateModel(
            name="DateAnswer",
            fields=[
                (
                    "id",
                    models.BigAutoField(
                        auto_created=True,
                        primary_key=True,
                        serialize=False,
                        verbose_name="ID",
                    ),
                ),
                ("created_at", models.DateTimeField(auto_now_add=True)),
                ("updated_at", models.DateTimeField(auto_now=True)),
                ("value", models.DateField()),
                (
                    "user",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.CASCADE,
                        related_name="%(class)ss",
                        to=settings.AUTH_USER_MODEL,
                    ),
                ),
            ],
            options={
                "abstract": False,
            },
        ),
        migrations.CreateModel(
            name="DateQuestionAnswer",
            fields=[
                (
                    "id",
                    models.BigAutoField(
                        auto_created=True,
                        primary_key=True,
                        serialize=False,
                        verbose_name="ID",
                    ),
                ),
                (
                    "answer",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.CASCADE,
                        related_name="question_answers",
                        to="form.dateanswer",
                    ),
                ),
                (
                    "question",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.CASCADE,
                        related_name="answers",
                        to="form.datequestion",
                    ),
                ),
            ],
        ),
        migrations.CreateModel(
            name="FileUploadAnswer",
            fields=[
                (
                    "id",
                    models.BigAutoField(
                        auto_created=True,
                        primary_key=True,
                        serialize=False,
                        verbose_name="ID",
                    ),
                ),
                ("created_at", models.DateTimeField(auto_now_add=True)),
                ("updated_at", models.DateTimeField(auto_now=True)),
                ("value", models.FileField(upload_to="answers/")),
                (
                    "user",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.CASCADE,
                        related_name="%(class)ss",
                        to=settings.AUTH_USER_MODEL,
                    ),
                ),
            ],
            options={
                "abstract": False,
            },
        ),
        migrations.CreateModel(
            name="FileUploadQuestionAnswer",
            fields=[
                (
                    "id",
                    models.BigAutoField(
                        auto_created=True,
                        primary_key=True,
                        serialize=False,
                        verbose_name="ID",
                    ),
                ),
                (
                    "answer",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.CASCADE,
                        related_name="question_answers",
                        to="form.fileuploadanswer",
                    ),
                ),
                (
                    "question",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.CASCADE,
                        related_name="answers",
                        to="form.fileuploadquestion",
                    ),
                ),
            ],
        ),
        migrations.CreateModel(
            name="MRForm",
            fields=[
                (
                    "id",
                    models.BigAutoField(
                        auto_created=True,
                        primary_key=True,
                        serialize=False,
                        verbose_name="ID",
                    ),
                ),
                ("name", models.CharField(max_length=200)),
                ("name_en", models.CharField(max_length=200, null=True)),
                ("name_nl", models.CharField(max_length=200, null=True)),
                ("description", models.TextField(blank=True, null=True)),
                ("description_en", models.TextField(blank=True, null=True)),
                ("description_nl", models.TextField(blank=True, null=True)),
                (
                    "config",
                    models.OneToOneField(
                        on_delete=django.db.models.deletion.CASCADE,
                        related_name="form",
                        to="form.mrformconfig",
                    ),
                ),
            ],
        ),
        migrations.CreateModel(
            name="NumberAnswer",
            fields=[
                (
                    "id",
                    models.BigAutoField(
                        auto_created=True,
                        primary_key=True,
                        serialize=False,
                        verbose_name="ID",
                    ),
                ),
                ("created_at", models.DateTimeField(auto_now_add=True)),
                ("updated_at", models.DateTimeField(auto_now=True)),
                ("value", models.DecimalField(decimal_places=2, max_digits=10)),
                (
                    "user",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.CASCADE,
                        related_name="%(class)ss",
                        to=settings.AUTH_USER_MODEL,
                    ),
                ),
            ],
            options={
                "abstract": False,
            },
        ),
        migrations.CreateModel(
            name="NumberQuestionAnswer",
            fields=[
                (
                    "id",
                    models.BigAutoField(
                        auto_created=True,
                        primary_key=True,
                        serialize=False,
                        verbose_name="ID",
                    ),
                ),
                (
                    "answer",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.CASCADE,
                        related_name="question_answers",
                        to="form.numberanswer",
                    ),
                ),
                (
                    "question",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.CASCADE,
                        related_name="answers",
                        to="form.numberquestion",
                    ),
                ),
            ],
        ),
        migrations.CreateModel(
            name="SelectAnswer",
            fields=[
                (
                    "id",
                    models.BigAutoField(
                        auto_created=True,
                        primary_key=True,
                        serialize=False,
                        verbose_name="ID",
                    ),
                ),
                ("created_at", models.DateTimeField(auto_now_add=True)),
                ("updated_at", models.DateTimeField(auto_now=True)),
                (
                    "value",
                    django.contrib.postgres.fields.ArrayField(
                        base_field=models.PositiveIntegerField(),
                        blank=True,
                        default=list,
                        size=None,
                    ),
                ),
                (
                    "user",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.CASCADE,
                        related_name="%(class)ss",
                        to=settings.AUTH_USER_MODEL,
                    ),
                ),
            ],
            options={
                "abstract": False,
            },
        ),
        migrations.CreateModel(
            name="SelectOption",
            fields=[
                (
                    "id",
                    models.BigAutoField(
                        auto_created=True,
                        primary_key=True,
                        serialize=False,
                        verbose_name="ID",
                    ),
                ),
                ("label", models.CharField(max_length=200)),
                ("label_en", models.CharField(max_length=200, null=True)),
                ("label_nl", models.CharField(max_length=200, null=True)),
                ("default_selected", models.BooleanField(default=False)),
                (
                    "question",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.CASCADE,
                        related_name="options",
                        to="form.selectquestion",
                    ),
                ),
            ],
        ),
        migrations.CreateModel(
            name="SelectQuestionAnswer",
            fields=[
                (
                    "id",
                    models.BigAutoField(
                        auto_created=True,
                        primary_key=True,
                        serialize=False,
                        verbose_name="ID",
                    ),
                ),
                (
                    "answer",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.CASCADE,
                        related_name="question_answers",
                        to="form.selectanswer",
                    ),
                ),
                (
                    "question",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.CASCADE,
                        related_name="answers",
                        to="form.selectquestion",
                    ),
                ),
            ],
        ),
        migrations.CreateModel(
            name="Step",
            fields=[
                (
                    "id",
                    models.BigAutoField(
                        auto_created=True,
                        primary_key=True,
                        serialize=False,
                        verbose_name="ID",
                    ),
                ),
                ("name", models.CharField(max_length=200)),
                ("name_en", models.CharField(max_length=200, null=True)),
                ("name_nl", models.CharField(max_length=200, null=True)),
                ("description", models.TextField(blank=True, null=True)),
                ("description_en", models.TextField(blank=True, null=True)),
                ("description_nl", models.TextField(blank=True, null=True)),
                (
                    "slug",
                    models.SlugField(
                        help_text="Used in the URL.", max_length=200, unique=True
                    ),
                ),
                (
                    "form_order",
                    models.PositiveIntegerField(
                        blank=True,
                        help_text="The order of this step within the form.",
                        null=True,
                    ),
                ),
                (
                    "parent_order",
                    models.PositiveIntegerField(
                        blank=True,
                        help_text="The order of this step within its parent step.",
                        null=True,
                    ),
                ),
                (
                    "form",
                    models.ForeignKey(
                        blank=True,
                        null=True,
                        on_delete=django.db.models.deletion.CASCADE,
                        related_name="steps",
                        to="form.mrform",
                    ),
                ),
                (
                    "parent",
                    models.ForeignKey(
                        blank=True,
                        null=True,
                        on_delete=django.db.models.deletion.CASCADE,
                        related_name="substeps",
                        to="form.step",
                    ),
                ),
            ],
        ),
        migrations.AddField(
            model_name="selectquestion",
            name="step",
            field=models.ForeignKey(
                on_delete=django.db.models.deletion.CASCADE, to="form.step"
            ),
        ),
        migrations.AddField(
            model_name="numberquestion",
            name="step",
            field=models.ForeignKey(
                on_delete=django.db.models.deletion.CASCADE, to="form.step"
            ),
        ),
        migrations.AddField(
            model_name="fileuploadquestion",
            name="step",
            field=models.ForeignKey(
                on_delete=django.db.models.deletion.CASCADE, to="form.step"
            ),
        ),
        migrations.AddField(
            model_name="datequestion",
            name="step",
            field=models.ForeignKey(
                on_delete=django.db.models.deletion.CASCADE, to="form.step"
            ),
        ),
        migrations.CreateModel(
            name="StepInfo",
            fields=[
                (
                    "id",
                    models.BigAutoField(
                        auto_created=True,
                        primary_key=True,
                        serialize=False,
                        verbose_name="ID",
                    ),
                ),
                (
                    "step",
                    models.OneToOneField(
                        blank=True,
                        null=True,
                        on_delete=django.db.models.deletion.CASCADE,
                        related_name="info",
                        to="form.step",
                    ),
                ),
            ],
        ),
        migrations.CreateModel(
            name="StepInfoQuestion",
            fields=[
                (
                    "id",
                    models.BigAutoField(
                        auto_created=True,
                        primary_key=True,
                        serialize=False,
                        verbose_name="ID",
                    ),
                ),
                ("text", models.CharField(max_length=200)),
                ("text_en", models.CharField(max_length=200, null=True)),
                ("text_nl", models.CharField(max_length=200, null=True)),
                ("link", models.URLField()),
                (
                    "step_info",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.CASCADE,
                        related_name="questions",
                        to="form.stepinfo",
                    ),
                ),
            ],
        ),
        migrations.CreateModel(
            name="StepInfoText",
            fields=[
                (
                    "id",
                    models.BigAutoField(
                        auto_created=True,
                        primary_key=True,
                        serialize=False,
                        verbose_name="ID",
                    ),
                ),
                ("text", models.TextField()),
                ("text_en", models.TextField(null=True)),
                ("text_nl", models.TextField(null=True)),
                (
                    "step_info",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.CASCADE,
                        related_name="texts",
                        to="form.stepinfo",
                    ),
                ),
            ],
        ),
        migrations.CreateModel(
            name="TextAnswer",
            fields=[
                (
                    "id",
                    models.BigAutoField(
                        auto_created=True,
                        primary_key=True,
                        serialize=False,
                        verbose_name="ID",
                    ),
                ),
                ("created_at", models.DateTimeField(auto_now_add=True)),
                ("updated_at", models.DateTimeField(auto_now=True)),
                ("value", models.TextField()),
                (
                    "user",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.CASCADE,
                        related_name="%(class)ss",
                        to=settings.AUTH_USER_MODEL,
                    ),
                ),
            ],
            options={
                "abstract": False,
            },
        ),
        migrations.CreateModel(
            name="TextQuestion",
            fields=[
                (
                    "id",
                    models.BigAutoField(
                        auto_created=True,
                        primary_key=True,
                        serialize=False,
                        verbose_name="ID",
                    ),
                ),
                ("text", models.CharField(max_length=200)),
                ("text_en", models.CharField(max_length=200, null=True)),
                ("text_nl", models.CharField(max_length=200, null=True)),
                (
                    "step_order",
                    models.PositiveIntegerField(
                        blank=True,
                        help_text="The order of this question within the step.",
                        null=True,
                    ),
                ),
                (
                    "description",
                    models.TextField(
                        blank=True,
                        help_text="Context about the question and its purpose, which will be shown to the user.",
                    ),
                ),
                (
                    "description_en",
                    models.TextField(
                        blank=True,
                        help_text="Context about the question and its purpose, which will be shown to the user.",
                        null=True,
                    ),
                ),
                (
                    "description_nl",
                    models.TextField(
                        blank=True,
                        help_text="Context about the question and its purpose, which will be shown to the user.",
                        null=True,
                    ),
                ),
                ("required", models.BooleanField(default=False)),
                ("placeholder", models.CharField(blank=True, max_length=200)),
                (
                    "placeholder_en",
                    models.CharField(blank=True, max_length=200, null=True),
                ),
                (
                    "placeholder_nl",
                    models.CharField(blank=True, max_length=200, null=True),
                ),
                ("lines", models.PositiveIntegerField(default=1)),
                (
                    "step",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.CASCADE, to="form.step"
                    ),
                ),
            ],
            options={
                "abstract": False,
            },
        ),
        migrations.CreateModel(
            name="TextQuestionAnswer",
            fields=[
                (
                    "id",
                    models.BigAutoField(
                        auto_created=True,
                        primary_key=True,
                        serialize=False,
                        verbose_name="ID",
                    ),
                ),
                (
                    "answer",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.CASCADE,
                        related_name="question_answers",
                        to="form.textanswer",
                    ),
                ),
                (
                    "question",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.CASCADE,
                        related_name="answers",
                        to="form.textquestion",
                    ),
                ),
            ],
        ),
        migrations.CreateModel(
            name="TrueFalseAnswer",
            fields=[
                (
                    "id",
                    models.BigAutoField(
                        auto_created=True,
                        primary_key=True,
                        serialize=False,
                        verbose_name="ID",
                    ),
                ),
                ("created_at", models.DateTimeField(auto_now_add=True)),
                ("updated_at", models.DateTimeField(auto_now=True)),
                ("value", models.BooleanField()),
                (
                    "user",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.CASCADE,
                        related_name="%(class)ss",
                        to=settings.AUTH_USER_MODEL,
                    ),
                ),
            ],
            options={
                "abstract": False,
            },
        ),
        migrations.CreateModel(
            name="TrueFalseQuestion",
            fields=[
                (
                    "id",
                    models.BigAutoField(
                        auto_created=True,
                        primary_key=True,
                        serialize=False,
                        verbose_name="ID",
                    ),
                ),
                ("text", models.CharField(max_length=200)),
                ("text_en", models.CharField(max_length=200, null=True)),
                ("text_nl", models.CharField(max_length=200, null=True)),
                (
                    "step_order",
                    models.PositiveIntegerField(
                        blank=True,
                        help_text="The order of this question within the step.",
                        null=True,
                    ),
                ),
                (
                    "description",
                    models.TextField(
                        blank=True,
                        help_text="Context about the question and its purpose, which will be shown to the user.",
                    ),
                ),
                (
                    "description_en",
                    models.TextField(
                        blank=True,
                        help_text="Context about the question and its purpose, which will be shown to the user.",
                        null=True,
                    ),
                ),
                (
                    "description_nl",
                    models.TextField(
                        blank=True,
                        help_text="Context about the question and its purpose, which will be shown to the user.",
                        null=True,
                    ),
                ),
                ("required", models.BooleanField(default=False)),
                ("default_value", models.BooleanField(default=False)),
                (
                    "step",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.CASCADE, to="form.step"
                    ),
                ),
            ],
            options={
                "abstract": False,
            },
        ),
        migrations.CreateModel(
            name="TrueFalseQuestionAnswer",
            fields=[
                (
                    "id",
                    models.BigAutoField(
                        auto_created=True,
                        primary_key=True,
                        serialize=False,
                        verbose_name="ID",
                    ),
                ),
                (
                    "answer",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.CASCADE,
                        related_name="question_answers",
                        to="form.truefalseanswer",
                    ),
                ),
                (
                    "question",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.CASCADE,
                        related_name="answers",
                        to="form.truefalsequestion",
                    ),
                ),
            ],
        ),
        migrations.AddConstraint(
            model_name="step",
            constraint=models.CheckConstraint(
                check=models.Q(
                    models.Q(
                        ("form_order__isnull", True),
                        ("parent_id__isnull", False),
                        ("parent_order__isnull", False),
                    ),
                    models.Q(
                        ("form_id__isnull", False),
                        ("form_order__isnull", False),
                        ("parent_order__isnull", True),
                    ),
                    _connector="OR",
                ),
                name="either_form_order_or_parent_order",
            ),
        ),
        migrations.AddConstraint(
            model_name="step",
            constraint=models.UniqueConstraint(
                fields=("parent", "parent_order"), name="unique_parent_order"
            ),
        ),
        migrations.AddConstraint(
            model_name="step",
            constraint=models.UniqueConstraint(
                fields=("form", "form_order"), name="unique_form_order"
            ),
        ),
        migrations.AddConstraint(
            model_name="selectquestion",
            constraint=models.UniqueConstraint(
                fields=("step_id", "step_order"),
                name="form_selectquestion_unique_step_order",
            ),
        ),
        migrations.AddConstraint(
            model_name="numberquestion",
            constraint=models.UniqueConstraint(
                fields=("step_id", "step_order"),
                name="form_numberquestion_unique_step_order",
            ),
        ),
        migrations.AddConstraint(
            model_name="fileuploadquestion",
            constraint=models.UniqueConstraint(
                fields=("step_id", "step_order"),
                name="form_fileuploadquestion_unique_step_order",
            ),
        ),
        migrations.AddConstraint(
            model_name="datequestion",
            constraint=models.UniqueConstraint(
                fields=("step_id", "step_order"),
                name="form_datequestion_unique_step_order",
            ),
        ),
        migrations.AddConstraint(
            model_name="textquestion",
            constraint=models.UniqueConstraint(
                fields=("step_id", "step_order"),
                name="form_textquestion_unique_step_order",
            ),
        ),
        migrations.AddConstraint(
            model_name="truefalsequestion",
            constraint=models.UniqueConstraint(
                fields=("step_id", "step_order"),
                name="form_truefalsequestion_unique_step_order",
            ),
        ),
    ]
